...
int main(int argc,char *argv[]){
    ...
    if(argc == 2){
        if(strcmp(argv[1],"--help") == 0){
            ...
            "ruta/infoNodo.csv, ruta/vecinos.csv, " <<
    ...
    agenteNaranja.join();
    for(int i = 0; i < datosNodos.size();1++){
    ...
    agenteAzul = std::thread(hiloAzul, &nodosIDS, &colasDeMensajes[0],
    argv[5],argv[6]);

    std::thread agenteVerde;
    agenteVerde = std::thread(hiloVerde,datosNodos.size()-1,datosNodos,
    &colasDeMensajes[0]);
