...
Cola<struct mensaje>* colasDeMensajes){

    unsigned int sock, length, fromlen;
    int n;
    struct sockaddr_in server;
    struct sockaddr_in from;
    struct mensaje mRecibido("No llegó mensaje (Esta es una plantilla)"
    , -1);
    ...
    char* ip;
    while (terminar!=1) {
        n = recvfrom(sock,&mRecibido,sizeof(mRecibido),MSG_DONTWAIT,
        (struct sockaddr *)&from,&fromlen);
        if (n > 0){
            mRecibido.ipFuente = from.sin_addr.s_addr;
            mRecibido.puertoFuente = from.sin_port;
            ip = (char*)tabla[0].IP.c_str();
            mRecibido.ipDestino = inet_addr(ip);
            mRecibido.puertoDestino = tabla[0].puerto;
...
void enviarV(int identificador, std::vector<datosNodo> tabla,
Cola<struct mensaje>* colasDeMensajes){
    ...
    while(terminar!=1){
        ...
        if(miMensaje.idDestino == -1){
            ...
            n=sendto(sock,&miMensaje,sizeof(miMensaje),0,
...
void hiloVerde(long unsigned int i2, std::vector<std::string> info,
Cola<struct mensaje>* colasDeMensajes){
    ...
    for(long unsigned int indice=0;indice<i2;indice++){
        conexiones[indice]=std::thread(enviarV,indice+1,tablaVecinos,
        colasDeMensajes);
    }
